...
export class GameServer {
  id: string; maxClients: number; connectedClients: number; server: Server; updatesPerSecond: number; lastTime: number; asteroidFieldSize: number; world: World; physics: PhysicsWorld; network: NetworkServer; bots: BotManager; mining: MiningSubsystem; combat: CombatSubsystem; fixedUpdate!: (delta: number) => number;
  constructor(id: string, maxClients: number, server: Server, physicsWorld: PhysicsWorld = new RapierPhysicsWorld(new NodeMeshProvider())) {
    ...
    this.lastTime = performance.now(); this.asteroidFieldSize = 4000; this.world = new World(); this.physics = physicsWorld; this.world.physics = this.physics;
    ...
    this.mining = new MiningSubsystem(); this.combat = new CombatSubsystem();
    this.world.addSubsystem(new RespawnSubsystem()).addSubsystem(this.combat).addSubsystem(this.mining);
    logger.info(`${this.id} running`);
  }
  async init(): Promise<void> {
    await this.physics.init();
    const asteroidCount = Math.max(50, Math.min(1000, Number(process.env.ASTEROID_COUNT) || 500));
    this.spawnAsteroids(asteroidCount); this.spawnVendor(); this.bots.reconcile(0);
    ...
    logger.info(`${this.id} simulation started (${asteroidCount} asteroids, ${this.updatesPerSecond} Hz)`);
  }
  ...
  tick(dt: number, time: number): void {
    ...
    this.awardKills(); this.physics.syncAsteroidScales?.(this.world); this.network.broadcastSpawned(this.mining.drainSpawned()); this.network.broadcastCollected(this.mining.drainCollected()); this.world.reap(); this.network.broadcast(this.world, time);
  }
  awardKills(): void {
    for (const kill of this.combat.drainKills()) {
      if (kill.killerId === null || kill.killerId === kill.victimId) continue;
      // The kill event is authoritative and was recorded before respawn/despawn.
      // Do NOT require the killer to still be alive: simultaneous death must not
      // erase the already-confirmed +100 credit reward.
      const killer = this.world.get(kill.killerId) as Ship | undefined;
      if (!killer) continue;
      awardKill(killer, kill.victimLevel);
    }
  }
  handlePlayerConnect(connection: Connection): void { logger.debug(`Adding player${connection.id} to ${this.id}`); this.connectedClients++; this.network.addConnection(connection); }
  spawnAsteroids(count: number): void {
    const rng = Utils.randomNumberGenerator(1);
    for (let i = 0; i < count; ++i) {
      const position = Utils.getRandomPosition(this.asteroidFieldSize, rng); const rotation = Utils.getRandomQuaternion(rng); const scaleValue = [10, 20, 40, 60, 120]; const scale = scaleValue[Math.floor(rng() * scaleValue.length)];
