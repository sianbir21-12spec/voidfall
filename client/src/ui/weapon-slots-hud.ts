...
// mouse button, mirroring how the ship actually fires (LMB/RMB).
const SLOTS = [
...
const ITEM_ICON: Record<number, string> = {
  ...
  [Items.LOCK_MISSILE]: 'lock-missile',
};
...
// cards. Reads the owned Ship's authoritative loadout, so the starter missile
// is visible immediately after spawn and remains visible after respawn/equip.
...
.vf-wslots__cell { display: flex; flex-direction: column; align-items: center; gap: calc(var(--u) * 0.32); }
.vf-wslots__card {
  ...
  box-shadow: inset 0 1px 0 rgba(255,255,255,0.06), inset 0 0 calc(var(--u) * 0.4) rgba(90,209,255,0.08);
  clip-path: polygon(calc(var(--u) * 0.42) 0, 100% 0, 100% 100%, 0 100%, 0 calc(var(--u) * 0.42));
  transition: border-color 140ms, box-shadow 140ms, transform 140ms;
}
.vf-wslots__card--empty { color: #3a4a6a; border-style: dashed; border-color: #2c3a54; background: linear-gradient(155deg, rgba(18,24,36,0.8), rgba(8,11,18,0.8)); }
.vf-wslots__card--firing { border-color: var(--cyan); box-shadow: 0 0 calc(var(--u) * 0.7) rgba(90,209,255,0.55), inset 0 0 calc(var(--u) * 0.5) rgba(90,209,255,0.25); transform: translateY(calc(var(--u) * -0.08)); }
.vf-wslots__glyph { display: grid; place-items: center; line-height: 0; }
.vf-wslots__bar { position: absolute; left: 0; right: 0; bottom: 0; height: calc(var(--u) * 0.2); background: linear-gradient(90deg, #2f96d6, var(--cyan)); box-shadow: 0 0 calc(var(--u) * 0.4) rgba(90,209,255,0.5); transition: opacity 140ms, box-shadow 140ms; }
.vf-wslots__card--empty .vf-wslots__bar { opacity: 0; }
.vf-wslots__card--firing .vf-wslots__bar { box-shadow: 0 0 calc(var(--u) * 0.6) rgba(90,209,255,0.9); }
.vf-wslots__bind { color: #8492ac; line-height: 0; }
@media (prefers-reduced-motion: reduce) { .vf-wslots__card { transition: none; } }
`;
